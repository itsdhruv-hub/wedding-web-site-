...
export default function Gallery({ t }) {
  ...
  return (
    <section id="gallery" className="py-10 md:py-16 bg-cream-light text-softBrown overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* Editorial Masonry Grid with Staggered Clip Reveals */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 auto-rows-[220px] sm:auto-rows-[240px]">
          {weddingConfig.gallery.map((img, idx) => (
            ...
            >
              <div onClick={() => openLightbox(idx)} className="w-full h-full relative">
                <img
                  src={img.url}
                  alt={img.caption}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-royal-maroon/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 pointer-events-none" />
      ...
      <AnimatePresence>
        {activeImageIndex !== null && (
          ...
          >
            ...
            <button
              onClick={prevImage}
              className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-gold hover:text-black transition-colors cursor-pointer z-10"
            ...
            <button
              onClick={nextImage}
              className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-gold hover:text-black transition-colors cursor-pointer z-10"
